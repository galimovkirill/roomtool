...
## ФАЗА 12 — Редактирование размеров в 3D

### TASK-032 — Ручки изменения размера элемента (resize handles)

**Промпт для Claude Code:**
```
Добавь 6 интерактивных ручек (handles) по центру каждой грани AABB выбранного
элемента. Пользователь захватывает ручку и тянет — элемент меняет размер вдоль
одной оси; противоположная грань остаётся на месте. Поведение аналогично
редактору PRO100 (синие квадратные точки по граням bounding-box).

---

## 1. Добавь сессию resize в src/store/sceneStore.ts

По аналогии с drag-сессией (`beginDrag` / `dragSelectionBy` / `endDrag`) добавь:

```typescript
// Дополнительное состояние (не экспортировать тип):
resizeItemId: string | null          // null = сессия не активна
resizeStartSnapshot: HistorySnapshot | null

beginResize(id: string): void
  // Если сессия уже активна — ничего.
  // Сохраняет текущий { items, groups } в resizeStartSnapshot.
  // Устанавливает resizeItemId = id.

resizeItemLive(id: string, dims: { width: number; height: number; depth: number }, position: [number, number, number]): void
  // НЕ пишет в историю.
  // Обновляет только dimensions и position у элемента с нужным id.
  // Игнорирует вызов, если resizeItemId !== id.

endResize(): void
  // Если resizeItemId null — ничего.
  // Кладёт resizeStartSnapshot в history (один undo-шаг на весь жест).
  // Сбрасывает resizeItemId = null, resizeStartSnapshot = null.
  // (Аналог endDrag(true) — всегда коммитит.)
```

Инициализируй оба новых поля значением `null` в начальном состоянии стора.

---

## 2. Создай src/components/scene/ResizeHandles.tsx
...
### 2a. Описание шести ручек

```typescript
interface HandleDef {
  id: string          // уникальный ключ для React key
  axis: 'x' | 'y' | 'z'
  sign: 1 | -1        // +1 = «дальняя» сторона, -1 = «ближняя»
  // Локальная позиция относительно центра элемента (в мм):
...
const HANDLE_DEFS: HandleDef[] = [
  ...
  { id: '-y', axis: 'y', sign: -1, localPos: (_, h)  => [0,     -h / 2,  0     ] },
  { id: '+z', axis: 'z', sign:  1, localPos: (_,_,d) => [0,      0,      d / 2 ] },
  { id: '-z', axis: 'z', sign: -1, localPos: (_,_,d) => [0,      0,     -d / 2 ] },
]
```
...
Каждая ручка — `<mesh>` с `<boxGeometry args={[24, 24, 24]} />`.  
Нормальное состояние: `<meshStandardMaterial color="#ffffff" />` + поверх — `<lineSegments>` с
`EdgesGeometry(BoxGeometry(24,24,24))` и `lineBasicMaterial color="#2563eb"`.  
При hover (`onPointerOver/Out`): заполнение меняется на `#2563eb`.

Используй `e.stopPropagation()` на всех pointer-событиях ручек, чтобы клик не
просачивался в SceneElement или Canvas.

### 2c. Механика drag через drag-plane

Для каждой ручки нужен `useRef` для отслеживания состояния drag-сессии. Используй
общий ref на уровне компонента `ResizeHandles`:
...
const dragState = useRef<{
  ...
  axisWorld: THREE.Vector3   // ось в мировом пространстве (с учётом rotationY)
} | null>(null)
```

**onPointerDown на ручке:**
```typescript
const { camera, gl, raycaster } = useThree()

// 1. Записать начало сессии
beginResize(item.id)

// 2. Запомнить начальные dims / position
const startDims = { ...item.dimensions }
const startPos: [number, number, number] = [...item.position]

// 3. Рассчитать мировую ось ручки (учитываем rotationY элемента)
//    Для оси Y rotationY не влияет — ось всегда [0,±1,0]
//    Для X/Z — вращаем локальную ось на rotationY:
...
// axisWorld — нормализованный вектор направления ручки в мировых координатах

// 4. Drag-plane: плоскость в мировом пространстве, параллельная взгляду камеры,
//    проходящая через мировую позицию ручки.
//    Нормаль плоскости = вектор от ручки к камере (проецируем на cam-dir без учёта handle axis)
const handleWorldPos = new THREE.Vector3(...item.position).add(
  new THREE.Vector3(...def.localPos(startDims.width, startDims.height, startDims.depth))
...
// 5. Найти начальную точку пересечения луча с плоскостью
const startIntersection = new THREE.Vector3()
// нормализованные экранные координаты из e.clientX / e.clientY
// ...raycaster.setFromCamera({ x, y }, camera)
// raycaster.ray.intersectPlane(dragPlane, startIntersection)

dragState.current = { handleId: def.id, axis: def.axis, sign: def.sign,
  startDims, startPos, startIntersection, dragPlane, axisWorld: localAxis }

// 6. Блокировать OrbitControls
window.dispatchEvent(new CustomEvent('transform-start'))
```

**Глобальные pointermove / pointerup через useEffect:**

Подпишись на `window.addEventListener('pointermove', ...)` и `window.addEventListener('pointerup', ...)` только пока идёт drag (когда `dragState.current !== null`). Очищай листенеры на pointerup и при unmount.

```typescript
// В pointermove:
if (!dragState.current) return
const { axis, sign, startDims, startPos, startIntersection, dragPlane, axisWorld } = dragState.current

// Нормализовать позицию мыши из event.clientX/Y
...
// Скалярная дельта вдоль оси (положительная = ручка движется ДАЛЬШЕ от центра)
const rawDelta = currentPoint.clone().sub(startIntersection).dot(axisWorld)

// Ограничения:
// Мин. размер детали = 1 мм
const MIN_DIM = 1
let delta = rawDelta

const dimKey = axis === 'x' ? 'width' : axis === 'y' ? 'height' : 'depth'
const newDim = Math.max(MIN_DIM, startDims[dimKey] + delta)
delta = newDim - startDims[dimKey]  // скорректированная дельта

// Для нижней ручки (-Y) — пол: низ элемента не уходит ниже y=0
// startBottom = startPos[1] - startDims.height / 2
// newPosY = startPos[1] - delta/2
// newHeight = startDims.height + delta
// constraint: newPosY - newHeight/2 >= 0
// → delta <= startPos[1] - startDims.height/2
if (axis === 'y' && sign === -1) {
  const maxDelta = startPos[1] - startDims.height / 2  // = startBottom (>= 0 уже гарантировано)
  delta = Math.min(delta, maxDelta)
}

// Пересчитываем после клампинга
const finalDim = Math.max(MIN_DIM, startDims[dimKey] + delta)

// Новые dimensions
const newDims = { ...startDims, [dimKey]: finalDim }

// Новая позиция центра: противоположная грань остаётся на месте.
// handle.sign = +1: ручка на стороне (+axis), т.е. «дальняя» грань двигается;
//   ближняя (−) фиксирована → центр смещается на +delta/2 по оси
// handle.sign = -1: «дальняя» (−) грань двигается;
//   ближняя (+) фиксирована → центр смещается на -delta/2 по оси
...
// Пол для произвольных ручек: центр не ниже halfHeight
...
Принимай в пропсы `isGltf: boolean`. Если `true` — рендери `null`
(GLTF-элементы имеют единый ползунок Scale, не отдельные оси).

---

## 3. Обнови src/components/scene/SceneElement.tsx

Условие показа ResizeHandles: одиночное выделение И элемент не является GLTF.

```typescript
// Уже есть в компоненте:
const selectedItemIds = useSceneStore(s => s.selectedItemIds)
...
// Внутри <group position={item.position} rotation={[0, item.rotationY, 0]}>:
{isSingleSelected && !isGltf && (
  <ResizeHandles item={item} isGltf={false} />
)}
```

ResizeHandles монтируется ВНУТРИ group SceneElement — ручки автоматически
наследуют позицию и поворот элемента. Все координаты ручек — локальные
(относительно центра элемента).

---

## 4. Не изменять SceneControls.tsx

`transform-start` / `transform-end` уже обрабатываются — дополнительных
изменений не нужно.
...
Добавь тесты для resize-сессии:
- `beginResize(id)` сохраняет снапшот (`resizeStartSnapshot !== null`, `resizeItemId === id`)
- `resizeItemLive(id, dims, pos)` обновляет dimensions и position, НЕ пишет в history
- `endResize()` после `resizeItemLive` — undo возвращает элемент к начальным dims/pos
- Двойной вызов `beginResize` не ломает снапшот первого вызова
- `resizeItemLive` с чужим id — игнорируется

---

## 6. Проверь в браузере (Playwright, скриншоты в tmp/)

1. Добавить боковую панель → выбрать → убедиться что 6 синих ручек по граням видны.
2. Захватить правую (+X) ручку → потянуть вправо → ширина растёт, левая грань
   остаётся на месте.
3. Захватить нижнюю (-Y) ручку → потянуть вниз до пола → элемент упирается в пол,
   не проваливается.
4. Захватить верхнюю (+Y) ручку → изменить высоту → в PropertiesPanel значение
   высоты обновляется после отпускания.
5. Undo (Ctrl+Z) → размер возвращается к предыдущему одним шагом.
6. house-plant-1 (GLTF) → выбрать → ручки НЕ появляются.
7. Повёрнутый элемент (rotationY = 90°) → ручки ориентированы правильно,
   дотащить до ограничения работает корректно.

Скриншоты: `tmp/task032-handles.png` (видны ручки), `tmp/task032-resize.png` (после
изменения размера).
```
...
| TASK-032 | Редактирование | Ручки изменения размера элемента (resize handles) | L | ✅ |
